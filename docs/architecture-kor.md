...
`known_attacks.json`의 1,490개 공격 프롬프트(Promptmap2 69개 + Purple Llama CyberSecEval 영어 245개 + 다국어 974개 + HuggingFace deepset 203개, 중복 제거)를 `sentence-transformers/all-MiniLM-L6-v2`로 벡터화하고 FAISS에 저장. 새 입력과의 코사인 유사도가 0.82 이상이면 차단. 18개 언어(영어, 한국어, 일본어, 중국어, 힌디어 등) 지원.
...
Gemini API를 6회 호출하고 67% 이상이 공격 카테고리로 분류되면 차단. 다중 분류(SAFE/JAILBREAK/INSTRUCTION_OVERRIDE/PROMPT_LEAK/ENCODING_EVASION/ROLEPLAY/SYSTEM_TOKEN_INJECTION)를 지원하며, 가장 많이 나온 카테고리를 metadata에 포함한다. LLM의 비결정성을 역이용한 독자적 설계. 429 Rate Limit 에러 시 최대 3회 재시도(백오프 1s/2s/3s)하며, 실패 시 SAFE로 폴백.
