...
[Stage 1: 정규식] 0.1ms, 무료 → 알려진 공격 패턴 탐지
    | (통과 시)
[Stage 2: 임베딩] ~50ms, 무료 → 1,490개 벡터와 의미 유사도 비교
    | (통과 시)
[Stage 3: Gemini] ~2s, 유료 → 6회 호출 다수결 + 공격 유형 분류
    | (통과 시)
...
counter_attack:
  enabled: true            # false로 바꾸면 역공격 비활성화 (단순 차단 모드)
  combo_mode: false
  combo_count: 2
  selector:
...
```python
# counter_engine 없이 파이프라인 생성
pipeline = (
    GuardrailPipeline()  # counter_engine 생략
    .add_stage(RegexStage())
    .add_stage(SanitizerStage())
)
# 공격 탐지 시 Decision.BLOCK이 반환됨 (역공격 응답 없음)
```
...
│   │   ├── embedding_stage.py   # Stage 2: 임베딩 (1,490개 벡터, 18개 언어)
...
- 정규식 패턴(15개)과 임베딩 시드(1,490개, 18개 언어)는 지속적으로 보강 중. 공개 데이터셋 추가로 확장 가능.
